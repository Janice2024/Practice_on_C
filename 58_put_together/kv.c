...
kvpair_t * lineToKV(char * line) {
  ...
  strncpy(p_kv->key, line, (keyLength));  //excluede \n
  p_kv->key[keyLength] = '\0';
  strncpy(p_kv->value, (equa + 1), valueLength - 2);  //excluede \n and\0
  p_kv->value[valueLength - 2] = '\0';
  // printf("p_kv->value:%s\n", p_kv->value);
  return p_kv;
}
